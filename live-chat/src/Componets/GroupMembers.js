        ...
        <AnimatePresence>
            ...
            >
                ...
                <motion.div
                    ...
                    whileTap={{ scale: 0.98 }}
                    className='ug-group-header'
                    onClick={createGroupChat}
                >
                    <p className='ug-title'>Create Group</p>
                </motion.div>
                {showToaster && <Toaster onClose={handleCloseToaster} severity="warning" message="Please add at least 2 members" />}
                ...
                <div className='ug-list'>
                    {userData.map((user) => {
                        if (user._id !== userInfo._id) {
                            ...
                            return (
                                <motion.div
                                    ...
                                    whileHover={{ scale: 1.02 }}
                                    whileTap={{ scale: 0.98 }}
                                    className={`lst-items ${userExists ? 'highlighted' : ''}`}
                        ...
                        return null; // Handle case where user is current user
                    })}
                </div>
            </motion.div>
        </AnimatePresence>
